...
public class BaseMessageActor extends UntypedAbstractActor {
	...
	public void preStart() {
		...
		if (this.actionPackageName != null) {
			...
			for (Map.Entry<Integer, Class<BaseMessageAction>> entry : mappingActionClassMap.entrySet()) {
				...
				if (mapping.isNet()) {
					int rpcNum = mapping.value();
					logger.info("regist netMsg handler for rpcNum = {}, rpcName = {}", rpcNum,
							RpcNameEnum.valueOf(rpcNum));
		...
		for (Method method : methods) {
			...
			if (mapping != null) {
				...
				for (int oneRpcNum : rpcNum) {
					...
					if (isNet) {
						logger.info("regist netMsg handler for rpcNum = {}", oneRpcNum);
	...
	public void onReceive(Object arg0) throws Throwable {
		...
		// 收到请求，先尝试寻找对应的Action类处理，找不到则在Actor中找对应处理函数
			...
			RpcNameEnum rpcNameEnum = RpcNameEnum.valueOf(rpcNum);
			try {
				...
				if (clazz != null) {
					...
					sender().tell(action.handleMessage(netMessage), ActorRef.noSender());
					...
					sendErrorToClient(netMessage, RpcErrorCodeEnum.ServerError_VALUE);
				}
			} catch (RpcErrorException e) {
				int errorCode = e.getErrorCode();
				RpcErrorCodeEnum errorCodeEnum = RpcErrorCodeEnum.valueOf(errorCode);
				logger.error("[resp {} error]: errorCode = {}, means = {}", rpcNameEnum == null ? rpcNum : rpcNameEnum,
						errorCode, errorCodeEnum);

				sendErrorToClient(netMessage, errorCode);
			} catch (Exception e) {
